...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if self.loss_function is LossFunction.MSE:
            return np.dot((y - self.predict(x)).T, y - self.predict(x))/y.shape[0]
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function is LossFunction.LogCosh:
            return np.sum(np.tanh(self.predict(x) - y) * x, axis = 0)
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        m_ = self.m / (1 - np.power(self.beta_1, self.iteration))
        v_ = self.v /(1 - np.power(self.beta_2, self.iteration))
        w_diff = -self.lr()/(np.sqrt(v_) + self.eps) * m_
        self.w +=  w_diff
